module.exports = class Command {
	...
	getUser(message, text, useMention = false) {
		...
		else return message.guild.users.cache.get(text) || message.guild.users.cache.get(text.split("@")[1].replace(">", "")) || message.guild.users.find(a => a.username.toLowerCase() === text.toLowerCase()).first()
	}
}